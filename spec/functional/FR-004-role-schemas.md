...
- Where a key is declared but the current extractor does not populate it (`routes`, `requires`, `carries`, `delivery`, `renders`, `triggers`, `associated_types`, `provider`, `versioning`, `exposes`, `registration`, `stability`, `records`, `endianness`, `serializes`, `thresholds`, `throttles`) — and likewise `relations` — the key SHALL be optional, so a record produced by today's extractor validates and a future extractor can fill it without a schema change.
