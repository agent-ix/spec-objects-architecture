...
- The manifest `semantic.mappings` SHALL declare `generalization`, the quire-rs FR-075 `ModelFeature::Generalization` token: without it, `quire.validate_document` refuses an interface's `specializes` relationship with `semantic.feature-not-extractable` naming `generalization` instead of lowering it; no other FR-152 member this module declares needs a mapping token beyond the three `semantic.mappings` already carried (`typed-table`, `sysml-fence`, `ocl-clause`) — in particular, no construct here declares `abstract`, so `abstract-types` is not added.
