...
- An interface SHALL declare its supertypes as `specializes` relationships to other interfaces: the verb is declared exactly as spec-objects-business declares it, `interface` admits `specializes: [interface]`, and the construct declares `supertypes` optional. The construct declares no `references` entry for `supertypes`: the semantic-ir reader already constrains a supertype reference to the same kind as the type declaring it, raising `CONSTRUCT_TARGET_KIND` otherwise (filament-core-data FR-141, `ix://agent-ix/filament-core-data/FR-141`), so a construct-level reference role would be redundant, and `references` admits only the closed member vocabulary filament-core-data FR-142 states, which `supertypes` is not in.
...
- **Upstream (vocabulary)**: `agent-ix/filament-core-data#172` defines the IR member names this requirement uses; filament-core-data FR-141 (`ix://agent-ix/filament-core-data/FR-141`) constrains a supertype reference to the same kind, and FR-142 (`ix://agent-ix/filament-core-data/FR-142`) closes the `references` member vocabulary a construct may name, which `supertypes` is outside of
