...
- The manifest `version` SHALL be `0.4.0`, because the emitted `$id` embeds it and the previous version was `0.3.0`; the minor bump records the additive [FR-007](./FR-007-systems-model-kinds.md) object types. The manifest `version` is the schema-contract version, not the Git release tag: the package version is computed from the tag (`poetry-dynamic-versioning`), and no requirement ties the two.
