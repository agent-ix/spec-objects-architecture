...
- The manifest `semantic` block SHALL carry exactly these keys and values: `contract_version: 1.0.0`, `semantic_core: 0.1.0`, `package: agent-ix/spec-objects-architecture`, `exports` listing every object type that ships a schema, `imports: {}`, `targets: [json-schema, markdown]`, `mappings: [typed-table, sysml-fence, ocl-clause, generalization]`, `compatibility_posture: strict`, `legacy_forms: warning`. [FR-007](./FR-007-systems-model-kinds.md) states why `generalization` is the fourth token and why no other token joins it.
