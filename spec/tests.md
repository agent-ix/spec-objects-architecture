...
reports **277 passed, 7 skipped, 17 xfailed**. The 7 skips are the
...
| FR-007 | FR-007-AC-1..12, FR-007-CON-1 | TC-100..TC-113 | 🚧 AC-7 is an expected failure on quire-rs#446; AC-11's validation half is an expected failure on quire-rs#448; AC-12's removal half is an expected failure on quire-rs#431 |
...
| TC-113 | `semantic.mappings` declares `generalization`; an interface with a `specializes` relationship extracts the edge and draws no `feature-not-extractable` diagnostic; a manifest copy with the token dropped reproduces that diagnostic | Integration | P0 | FR-007-AC-12 | ✅ the declared half passes; the removal half is an expected failure on quire-rs#431 and passes against a wheel built from quire-rs main (PR #432) |
...
checks that `ModelFeature::Generalization` (the same `agent-ix/quire-rs#431`,
landed by PR #432) refuses a `specializes` relationship with
`semantic.feature-not-extractable` when `generalization` is absent from
`semantic.mappings`; an engine that does not carry that gate yet drops the
relationship silently instead, and TC-113's removal half pins that exact
behavior rather than assuming the gate.

Measured with `make test` on 2026-09-18: the `pypi.ix` quire 0.46.0 wheel
gives 277 passed, 7 skipped, 17 xfailed. At commit `2f27953`, where TC-054 and
