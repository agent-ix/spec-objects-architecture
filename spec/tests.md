...
| TC-110 | An interface declares supertypes: `specializes` equals the spec-objects-business declaration pinned with its source revision, `interface` admits it to `interface`, the construct names `supertypes` optional with no `references` entry for it, the edge extracts and is not disallowed, and a disallowed verb warns | Integration | P0 | FR-007-AC-10 | ✅ |
