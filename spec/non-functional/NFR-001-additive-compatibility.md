...
is asserted rather than assumed. The one intended break is the object-id rule:
an object id never carries a hyphen, so a 0.2.0 artifact whose id does draws
exactly one frontmatter error at `id` and validates once the id is in word
form. The checked-in 0.2.0 set is a frozen capture and keeps its original ids.
...
unchanged, and the only `allowed_links` and `roles` changes are the named
`interface` additions.
...
| NFR-001-AC-2 | Every skeleton of the checked-in 0.2.0 set, with its id in word form, validates under 0.4.0 with 0 error findings; with its frozen hyphenated id, its only error finding is the frontmatter `id` pattern of [FR-003](../functional/FR-003-semantic-manifest-contract.md). | Test |
