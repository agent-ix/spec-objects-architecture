...
* **2026-09-22** — Retired StR-001-VC-2 and TC-006. Skeletons are no longer rendered as templates (dropped: it saved neither time nor tokens), so every skeleton is placeholder-free (TC-056) and the two `minijinja-cli` tests only proved a third-party tool returns an unchanged file; their validate half repeated TC-050. StR-001 and spec.md no longer name a template generator.
